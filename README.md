...
- **传**：右上角「登录以上传」—— 邮箱＋密码直接登录（推荐，路上不依赖收邮件）；
  密码留空则发一条 magic link 到邮箱。之后「上传照片」可一次选多张。
  注意 magic link 的落地页由 Supabase 后台 Authentication → URL Configuration 的
  Site URL 决定，需设成正式域名，否则会跳到 localhost。
- **删**：登录后点「管理」，每张图右上角出现 ×。
